...
- `offline`: replay native Trajectory v1.0 HDF5 as Observation v1.0 through the same `eval_one_episode` loop. Default episode directory is `../data/<bench_name>/<task_name>/<env_cfg_type>/data/`; override with `--data_path` or `XPL_OFFLINE_DATA_PATH`.
